...
import { ScenarioType, scenarios } from "./scenarios.js";

const sleep = (ms = 2000) => new Promise((resolve) => setTimeout(resolve, ms));

const handleColors = (scenario: ScenarioType) => {
...
const presentScenario = async (scenario: ScenarioType) => {
  handleColors(scenario);

  const answers: { choice: string } = await inquirer.prompt([
    {
      type: "list",
      name: "choice",
...
const startGame = async () => {
  ...
  while (currentScenario) {
    ...
    const currentChoice = currentScenario.choices.find(
    ...
    if (currentChoice)
      currentScenario = scenarios.find(
        (scenario) => scenario.name === currentChoice.nextScenario
      );
  }
